...
(function() {
  ...
  VisualOrganism = (function() {
    ...
    VisualOrganism.prototype.buildScene = function() {
      var ballGlowMaterial, ballGlowSprite, factor, factor3d, factorColor, j, len, now, ref, roomColor;
      now = new Date();
      this.lastLoop = 0;
      this.thisLoop = 0;
      this.fps = 0;
    ...
    VisualOrganism.prototype.animate = function() {
      var _moveFactor, ball, cube, factor, factor3d, j, k, l, len, len1, len2, len3, line, lineGeometry, lineMaterial, m, megaCube, ref, ref1, ref2, ref3;
      ...
      _moveFactor = (function(_this) {
        return function(factorType) {
          var factor, factor3d;
    ...
    VisualOrganism.prototype.runQueue = function() {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        action = ref[j];
        results.push(action.fn.call(this, action.args));
      }
      return results;
    };

    VisualOrganism.prototype.onIteration = function(influenceInfo) {
      var ball, ballScaleTo, cell, cells, disharmony, factorChangeAvg, factorChangeSum, j, k, l, len, len1, len2, newBallScale, newPos, node, organism, ref, ref1, relativeDisharmony, results, tweenFrom, tweenTo;
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (this.frame % 4 === 0) {
          ...
          this._tweenSomething(ball.ball3d.position, tweenFrom, tweenTo, 100, (function(_this) {
            return function() {
      ...
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        ...
        if (this.frame - ball.ball3d.userData.bornAt > 30) {
          factorChangeSum = 0;
          for (l = 0, len2 = cells.length; l < len2; l++) {
            cell = cells[l];
          ...
          results.push(this._tweenSomething(ball.ball3d.scale, ball.ball3d.scale.clone(), ballScaleTo, 200));
        } else {
          results.push(void 0);
        }
      }
    ...
    VisualOrganism.prototype.createBalls = function(numBalls) {
      var ball, ball3d, ballGeometry, ballMaterials, i, j, ref, results;
      results = [];
      for (i = j = 0, ref = numBalls - 1; 0 <= ref ? j <= ref : j >= ref; i = 0 <= ref ? ++j : --j) {
        ball = {
        ...
        results.push(ball.ball3d = ball3d);
      }
      return results;
    };
    ...
    VisualOrganism.prototype.onCompareNodes = function(compareData) {
      ...
      ref = compareData.nodes;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        node = ref[j];
        results.push(this._animateComparingBall(this.balls[node.nodeId]));
      }
      return results;
    };
    ...
    VisualOrganism.prototype._animateComparingBall = function(ball) {
      ...
      this._tweenBallColor(ball, ballColor.clone(), this.opts.ballColorCompare.clone(), 200, (function(_this) {
        return function() {
      ...
      return this._tweenSomething(ball.ball3d.position, ball.ball3d.position.clone(), newPos, 30, (function(_this) {
        return function() {
    ...
    VisualOrganism.prototype.onInfluenceFactorAfter = function(influenceData) {
      var _afterFactorAnimation, factor, factor3d, newColor;
      factor = influenceData.factor.factor;
      factor3d = this.factors[factor.factorType];
      factor3d.userData.hover = false;
      factor3d.userData.isModifying = true;
      _afterFactorAnimation = (function(_this) {
        return function() {
    ...
    VisualOrganism.prototype._animateInfluencedBall = function(ball) {
      ...
      this._tweenSomething(target, target.clone(), this.opts.ballColorInfluence.clone(), 200, (function(_this) {
        return function() {
      ...
      return this._tweenSomething(ball.ball3d.position, ball.ball3d.position.clone(), newPos, 200, (function(_this) {
        return function() {
    ...
    VisualOrganism.prototype._replaceCubesWithMegaCube = function() {
      var cubeColor, cubeEndIndex, cubeGeometry, cubeInfo, cubeMaterial, cubeStartIndex, j, len, megaCube, megaCubePos, oldCubes;
      ...
      for (j = 0, len = oldCubes.length; j < len; j++) {
        cubeInfo = oldCubes[j];
    ...
    VisualOrganism.prototype._removeCube = function(cubeInfo, destination, megaCube) {
      ...
      return this._tweenSomething(cubePos, cubePos.clone(), destination, 1000, (function(_this) {
        return function() {
    ...
    VisualOrganism.prototype._updateInstaCubes = function() {
      var cubeInfo, j, len, ref, results;
      ref = this.instaCubes;
      results = [];
      for (j = 0, len = ref.length; j < len; j++) {
        cubeInfo = ref[j];
        results.push(cubeInfo.cube.rotation.x += cubeInfo.rotateSpeed);
      }
      return results;
    };
    ...
    VisualOrganism.prototype._tweenSomething = function(something, from, to, duration, callback) {
      ...
      tween.onUpdate(function() {
        ...
        for (j = 0, len = ref.length; j < len; j++) {
          key = ref[j];
          results.push(something[key] = this[key]);
        }
        return results;
      });
      if (callback) {
